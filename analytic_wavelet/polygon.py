...
def _det(x_vertices, y_vertices):
    ...
    """
    x_vertices = np.asfarray(x_vertices)
    y_vertices = np.asfarray(y_vertices)
    x_prev = np.concatenate(([x_vertices[-1]], x_vertices[:-1]))
    y_prev = np.concatenate(([y_vertices[-1]], y_vertices[:-1]))
    return np.sum(y_vertices * x_prev - x_vertices * y_prev, axis=0)
...
class Polygon:
    ...
    def is_inside(self, x_point, y_point, smalld=1e-12):
        ...
        """
        x_point = np.asfarray(x_point)
        y_point = np.asfarray(y_point)
        # Scalar to array
        if x_point.shape is tuple():
            x_point = np.array([x_point], dtype=float)
            y_point = np.array([y_point], dtype=float)
            scalar = True
        else:
            scalar = False
        # Check consistency
        if x_point.shape != y_point.shape:
        ...
        snear = np.ma.masked_all(x_point.shape, dtype=bool)
        # Initialize arrays
        mindst = np.ones_like(x_point, dtype=float) * np.inf
        j = np.ma.masked_all(x_point.shape, dtype=int)
        x = self.x
        y = self.y
        n = len(x) - 1  # Number of sides/vertices defining the polygon
        # Loop over each side defining polygon
        for i in range(n):
            d = np.ones_like(x_point, dtype=float) * np.inf
        ...
        # outside polygon.
        area = _det([x[j], x[j + 1], x_point], [y[j], y[j + 1], y_point])
        mindst[snear] = np.copysign(mindst, area)[snear]
